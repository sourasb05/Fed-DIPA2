...
class User():
    ...
    def __init__(self,device, args, id, exp_no, current_directory, wandb):
        ...
        self.train_df = self.mega_table.sample(n=train_size, random_state=0)
        rem_df = self.mega_table.drop(self.train_df.index)
        ...
        self.train_df.to_csv("%s/train_%d.csv" % (dataset_files_dir, int(self.id)), index=False)
        ...
        if not args.test:
            train_dataset = ImageMaskDataset(self.train_df, feature_folder, self.input_channel, image_size, flip = True)
