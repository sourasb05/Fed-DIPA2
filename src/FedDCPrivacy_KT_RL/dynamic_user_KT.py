...
class FedDCPrivacy_KT_RL_User():
    ...
    def __init__(self,device, args, id, exp_no, current_directory, wandb):
        ...
        self.no_lamda=args.lamda_sim_sta  ##tradeoff between similarity and stability
    ...
    def save_model(self, glob_iter, epoch, current_loss):
        ...
        #model_path = ( f"{self.current_directory}/models/{self.algorithm}/local_model/CFedDC_KT_RL_rl1_C{self.cluster_number}/{self.id}/delta_{self.delta}_kappa_{self.kappa}_lambda_{self.lambda_min}_{self.lambda_max}_GE_{self.num_glob_iters}_LE_{self.local_iters}"
# )
        model_path = ( f"{self.current_directory}/models/{self.algorithm}/local_model/CFedDC_KT_RL_rl1_C{self.cluster_number}/{self.id}/delta_{self.delta}_kappa_{self.kappa}_no_lambda_GE_{self.num_glob_iters}_LE_{self.local_iters}"
)
        if not os.path.exists(model_path):
            os.makedirs(model_path)
    ...
    def train(self, user_type, cluster_model,t):
        # print(f"user id : {self.id}")
        if user_type == "RF":
        ...
        for iter in range(self.local_iters):
            for batch in self.train_loader:
                ...
                loss = loss + self.no_lamda * (self.lamda*loss1 + (1-self.lamda)*loss2)
                loss.backward()
                self.optimizer.step()
    ...
    def exchange(self, rl_user, cluster_model, t):
        for rl_param, param in zip(rl_user.local_model.parameters(), self.local_model.parameters()):
