...
from TrainModels.trainmodels import PrivacyModel, PrivacyModel_MOON
from sklearn.preprocessing import LabelEncoder
from torch.utils.data import DataLoader
...
class Base_user:
    ...
    def __init__(self,device, args, id, exp_no, current_directory, wandb):
        ...
        if self.algorithm == "MOON":
            self.local_model = PrivacyModel_MOON(input_dim=self.input_dim).to(self.device)
            self.global_model = copy.deepcopy(self.local_model)
            self.prev_model = copy.deepcopy(self.local_model)
    ...
    def load_model(self):
        models_dir = f"./models/{self.algorithm}/global_model/"
